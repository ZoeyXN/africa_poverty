...
DHS_TFRECORDS_SA_5COUNTRY_PATH_ROOT = os.path.join(ROOT_DIR, 'data/South_Asia_5_countries/dhs_tfrecords-3_year_period-Landsat_DMSP_VIIRS')
...
def dhs_sa() -> np.ndarray:
    '''Gets a list of paths to all TFRecord files comprising the DHS dataset.

    Returns: np.array of str, sorted paths to TFRecord files
    '''
    splits = ['train', 'val']
    dataset = 'DHS_SA_5country'

    survey_names = SURVEY_NAMES[dataset]
    tfrecord_paths = []
    for s in splits:
        print(f'split {s}: {survey_names[s]}')
        for country_year in survey_names[s]:
            glob_path = os.path.join(
                DHS_TFRECORDS_SA_5COUNTRY_PATH_ROOT, country_year + '*', '*.tfrecord.gz')
            tfrecord_paths += glob(glob_path)
    assert len(tfrecord_paths) == SIZES[dataset]['all'], f"{len(tfrecord_paths)} {SIZES[dataset]['all']}"
...
def dhs_ooc(dataset: str, split: str) -> np.ndarray:
    ...
    for s in splits:
        for country_year in survey_names[s]:
            ...
            tfrecord_paths += glob(glob_path)
    assert len(tfrecord_paths) == SIZES[dataset][split], f'{len(tfrecord_paths)} {SIZES[dataset][split]}'
    return np.sort(tfrecord_paths)
